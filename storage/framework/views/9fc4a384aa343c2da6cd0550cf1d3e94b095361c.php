		...
		<table class="table-bordered table table-hover">
			...
			<?php $__currentLoopData = $data; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $value): $__env->incrementLoopIndices(); $loop = $__env->getFirstLoop(); ?>
				<tr>
					...
					<td>
						<?php if($value->type): ?>
							<a href="javascript:;" onclick="establish(<?php echo e($value->id); ?>,<?php echo e($value->typeid); ?>)" data-toggle="modal" data-target="#establish" class="glyphicon glyphicon-plus"></a>
						<?php else: ?>
...
<script>
    ...
    function fenpei(){
        ...
        $.post("/admin/course/fenpei",{str:str,'_token':'<?php echo e(csrf_token()); ?>'},function(data){
            // 判断data
            if (data==1) {
    ...
    function tianjia() {
        ...
        $.post("/admin/course/tianjia",{str:str,'_token':'<?php echo e(csrf_token()); ?>'},function(data){
            // 判断data
            if (data==1) {
                window.location.reload();
            }else if(data){
    ...
    function establish(id,typeid){
        $.post("/admin/course/establish",{id:id,typeid:typeid,'_token':'<?php echo e(csrf_token()); ?>'},function(data){
            if (data) {
                $("#body2").html(data);
            };
        });
